...
//     document.getElementById('best-match-label').textContent = t.topMatchBadge;
//     document.getElementById('second-match-label').textContent = t.secondMatchBadge;
//     document.getElementById('nearby-label').textContent = t.nearestBadge;
//     // document.getElementById('match-score-text') // Removed.textContent = t.matchScoreText;
//     // document.getElementById('match-score-text-2') // Removed.textContent = t.matchScoreText;
// }
...
function updateUILanguage() {
    ...
    // Welcome screen
    updateElement('welcome-title', t.welcomeTitle);
    updateElement('welcome-subtitle', t.welcomeSubtitle);
    updateElement('info-card-1-title', t.infoCard1Title);
    updateElement('info-card-1-text', t.infoCard1Text);
    updateElement('info-card-2-title', t.infoCard2Title);
    updateElement('info-card-2-text', t.infoCard2Text);
    updateElement('info-card-3-title', t.infoCard3Title);
    updateElement('info-card-3-text', t.infoCard3Text);
    updateElement('welcome-description', t.welcomeDescription);
    updateElement('start-btn', t.startBtn);
    updateElement('change-lang-btn', t.changeLangBtn);
    
    // Quiz screen
    updateElement('back-text', t.backText);
    updateElement('prev-text', t.prevText);
    updateElement('next-text', t.nextText);
    
    // Results screen
    updateElement('results-title', t.resultsTitle);
    updateElement('results-subtitle', t.resultsSubtitle);
    updateElement('best-match-label', t.bestMatchLabel || 'Best Match');
    updateElement('second-match-label', t.secondMatchLabel || 'Great Alternative');
    updateElement('nearby-label', t.nearbyLabel || 'Close to You');
    updateElement('restart-text', t.restartText);
}
...
// Enhanced displayResults for new HTML structure
function displayResults(top, second, nearest) {
    document.getElementById('quiz-screen').classList.remove('active');
    ...
    // Display top match - NO PERCENTAGE
    document.getElementById('uni-1-name').textContent = top.university.name;
    document.getElementById('uni-1-location').textContent = top.university.location;
    document.getElementById('uni-1-description').textContent = top.university.description;
    displayCareers('uni-1-careers', top.university.careers);
    
    // Display second match - NO PERCENTAGE
    document.getElementById('uni-2-name').textContent = second.university.name;
    document.getElementById('uni-2-location').textContent = second.university.location;
    document.getElementById('uni-2-description').textContent = second.university.description;
    displayCareers('uni-2-careers', second.university.careers);
    
    // Display nearest - NO PERCENTAGE
    document.getElementById('uni-3-name').textContent = nearest.university.name;
    document.getElementById('uni-3-location').textContent = nearest.university.location;
    document.getElementById('uni-3-description').textContent = nearest.university.description;
    displayCareers('uni-3-careers', nearest.university.careers);
    
    // Update labels
    const restartEl = document.getElementById('restart-text');
    if (restartEl) restartEl.textContent = t.restartText || 'Retake Quiz';
}


// Update UI language for intro page
function updateIntroLanguage() {
    const t = translations[currentLang];
    const elem = (id) => document.getElementById(id);
    
    if (elem('school-name')) elem('school-name').textContent = t.schoolName || 'LHF Kaiserhof Meran';
    if (elem('intro-heading')) elem('intro-heading').textContent = t.introHeading || t.welcomeTitle;
    if (elem('intro-desc-1')) elem('intro-desc-1').innerHTML = t.introDesc1 || t.welcomeDescription;
    if (elem('intro-desc-2')) elem('intro-desc-2').textContent = t.introDesc2 || '';
    if (elem('feature-1-title')) elem('feature-1-title').textContent = t.feature1Title || '35 Questions';
    if (elem('feature-1-text')) elem('feature-1-text').textContent = t.feature1Text || 'Personalized assessment';
    if (elem('feature-2-title')) elem('feature-2-title').textContent = t.feature2Title || '120 Universities';
    if (elem('feature-2-text')) elem('feature-2-text').textContent = t.feature2Text || '30 from each country';
    if (elem('feature-3-title')) elem('feature-3-title').textContent = t.feature3Title || 'Career Insights';
    if (elem('feature-3-text')) elem('feature-3-text').textContent = t.feature3Text || 'See opportunities';
    if (elem('start-journey-btn')) elem('start-journey-btn').textContent = t.startJourneyBtn || 'Begin Your Journey';
}

// Call this when language is selected

// Enhanced displayCareers for split layout
